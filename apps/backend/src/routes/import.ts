...
router.post(
  ...
  authenticate,
  rateLimit(10, 60000),
  validateBody(importValidateSchema),
  async (
    req: AuthenticatedRequest & { validated?: { body: ImportValidateInput } },
    res: Response
  ): Promise<void> => {
    try {
      const { type, data, metadata } = req.validated?.body || {
      ...
      if (type === 'employees') {
        ...
        existingEmployees.forEach((doc) => {
          ...
          warnings.push(
            `Employee ${existingEmail} already exists and will be updated`
          );
        });
      } else if (type === 'hours') {
        ...
        const employeeEmails = [
          ...new Set(
            data.map((row) => (row as { employeeEmail?: string }).employeeEmail)
          ),
        ];
        const existingEmployees = await db
          .collection('users')
...
router.post(
  ...
  authenticate,
  rateLimit(5, 60000),
  validateBody(employeeImportSchema),
  async (
    req: AuthenticatedRequest & { validated?: { body: EmployeeImportInput } },
    res: Response
  ): Promise<void> => {
    try {
      const { data, metadata } = req.validated?.body || {
        data: [],
        metadata: {
          fileName: '',
          totalRows: 0,
      ...
      for (const row of data) {
        try {
          ...
          if (!existingEmployees.empty) {
            ...
            if (!existingDoc) {
              throw new Error(
                `Employee document not found for email: ${row.email as string}`
              );
            }
...
router.post(
  ...
  authenticate,
  rateLimit(10, 60000),
  validateBody(hoursImportSchema),
  async (
    req: AuthenticatedRequest & { validated?: { body: HoursImportInput } },
    res: Response
  ): Promise<void> => {
    try {
      const { data, metadata } = req.validated?.body || {
      ...
      for (const row of data) {
        try {
          ...
          console.error(
            `Error processing hours for ${row.employeeEmail}:`,
            error
          );
          failed.push({
            email: row.employeeEmail,
...
router.get(
  ...
  async (
    req: AuthenticatedRequest & {
      validated?: { query: ImportHistoryQueryInput };
    },
    res: Response
  ): Promise<void> => {
    try {
