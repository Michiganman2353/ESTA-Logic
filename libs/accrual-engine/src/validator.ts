 ...
 */
export function validateHoursWorked(hoursWorked: number): {
  valid: boolean;
  error?: string;
} {
  if (hoursWorked < 0) {
  ...
  if (hoursWorked > 24) {
    return {
      valid: false,
      error: 'Hours worked cannot exceed 24 hours in a day',
    };
  }
...
export function validateAccrualRequest(
  ...
  if (yearlyAccrued > maxCap) {
    return {
      valid: false,
      error: `Yearly accrued hours exceed maximum cap of ${maxCap}`,
    };
  }
