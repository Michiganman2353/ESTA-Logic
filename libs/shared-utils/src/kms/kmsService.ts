...
function getKMSConfig(): KMSConfig {
  ...
  if (!config.projectId) {
    throw new Error(
      'GCP_PROJECT_ID or FIREBASE_PROJECT_ID must be set for KMS operations'
    );
  }
...
class KMSService {
  ...
  async getPublicKey(version?: string): Promise<KMSKeyPair> {
    ...
    try {
      ...
      throw new Error(
        `KMS public key retrieval failed: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
