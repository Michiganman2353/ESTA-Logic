...
if (missing.length > 0) {
  ...
  console.error('');
  console.error(
    'Set these in your environment, in Vercel project settings, or as GitHub Actions secrets.'
  );
  console.error(
    'For local development, create a .env with the VITE_ prefixed keys.'
  );
  console.error('');
  console.error(
    '⚠️  NOTE: REACT_APP_* and unprefixed FIREBASE_* variables are NO LONGER SUPPORTED.'
  );
  console.error(
    '    The monorepo exclusively uses VITE_* prefix for all frontend environment variables.'
  );
  process.exit(1);
}

console.log(
  '✅ Firebase environment variables check passed. Found the following keys:'
);
