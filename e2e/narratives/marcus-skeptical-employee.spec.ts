...
test.describe("Marcus's Journey: Skeptical Employee to Believer", () => {
  test.describe('Act 1: Skepticism (Initial Contact)', () => {
    test('Employee portal is accessible and not intimidating', async ({
      page,
    }) => {
      ...
      const buttonVisible = await employeeButton
  ...
  test.describe('Act 2: Surprise (First Login)', () => {
    test('Marcus sees clean, simple interface that surprises him positively', async ({
      page,
    }) => {
    ...
    test('Mobile portal shows balance immediately - no confusion', async ({
      page,
    }) => {
      ...
      const balanceDisplay = page.locator(
        '[data-testid*="balance"], [class*="balance"], text=/\\d+(\\.\\d+)?\\s*(hours|hrs)/i'
      );

      // Balance information should be immediately visible
      const hasBalance = await balanceDisplay
        .first()
        .isVisible({ timeout: 10000 })
        .catch(() => false);
  ...
  test.describe('Act 3: Validation (Trust Building)', () => {
    test('Marcus can see how balance was calculated - transparency wins', async ({
      page,
    }) => {
      await page.goto('/');
      await page.waitForLoadState('domcontentloaded');
      ...
      const hasDetailsLink = await page
        .locator(
          '[data-testid*="detail"], [data-testid*="breakdown"], text=/how|details|view|calculate/i'
        )
        .first()
    ...
    test('Security indicators show data is protected', async ({ page }) => {
      ...
      const hasSecurityIndicators =
        /encrypted|secure|private|protected|safe|privacy/i.test(bodyText || '');

      // Log for debugging
      if (!hasSecurityIndicators) {
        console.log(
          'No security indicators found. Body content sample:',
    ...
    test('Legal references show legitimacy', async ({ page }) => {
      ...
      const bodyText = await page.textContent('body');
      const hasLegalReferences =
        /Michigan|ESTA|law|legal|compliance|compliant|accurate/i.test(
          bodyText || ''
        );
      ...
      if (!hasLegalReferences) {
        console.log(
          'No legal references found. Body content sample:',
          bodyText?.substring(0, 200)
        );
      }
    ...
    test('Marcus can view accrual history - validation through data', async ({
      page,
    }) => {
      await page.goto('/');
      await page.waitForLoadState('domcontentloaded');
      ...
      const hasHistory = await page
        .locator(
          '[data-testid*="history"], [data-testid*="timeline"], text=/history|timeline|log/i'
        )
        .first()
        .isVisible({ timeout: 5000 })
        .catch(() => false);
  ...
  test.describe('Act 4: Empowerment (Taking Control)', () => {
    test('Marcus can request time off from phone in 30 seconds', async ({
      page,
    }) => {
      await page.goto('/');
      await page.waitForLoadState('domcontentloaded');
  ...
  test.describe('Act 5: Advocacy (Active User)', () => {
    test('Marcus appreciates transparency and recommends to coworkers', async ({
      page,
    }) => {
      ...
      const hasEmployeeCentric =
        /your balance|your time|your hours|you have|you earned|employee|track|manage/i.test(
          bodyText || ''
        );
      ...
      if (!hasEmployeeCentric) {
        console.log(
          'No employee-centric language found. Body content sample:',
          bodyText?.substring(0, 200)
        );
      }
...
test.describe("Marcus's Journey: Employee-Specific UX Principles", () => {
  test('Mobile-first design works for on-the-go employees', async ({
    page,
  }) => {
    // Set mobile viewport
  ...
  test('No jargon - plain language for all education levels', async ({
    page,
  }) => {
    await page.goto('/');
    ...
    const hasExcessiveJargon =
      /statutory compliance framework|regulatory adherence verification|aggregate accrual computation/i.test(
        bodyText || ''
      );
  ...
  test('Self-service reduces HR dependency', async ({ page }) => {
    ...
    const hasSelfService = await page
      .locator(
        '[data-testid*="request"], [data-testid*="balance"], [data-testid*="history"]'
      )
      .first()
      .isVisible({ timeout: 5000 })
      .catch(() => false);
  ...
  test('Privacy respected - employees see only their own data', async ({
    page,
  }) => {
    await page.goto('/', { waitUntil: 'domcontentloaded' });
    ...
    const hasPrivacy =
      /privacy|private|your data|secure|protected|safe|personal/i.test(
        bodyText || ''
      );
    ...
    if (!hasPrivacy) {
      console.log(
        'No privacy messaging found. Body content sample:',
        bodyText?.substring(0, 200)
      );
    }
  ...
  test('Trust signals combat employee skepticism', async ({ page }) => {
    ...
    if (!hasMultipleTrustSignals) {
      console.log(
        'Insufficient trust signals. Trust signal count:',
        trustSignals.filter(Boolean).length
      );
    }
