...
test.describe("Sarah's Journey: Small Business Owner Compliance Story", () => {
  test.describe('Act 2: Discovery & Hope (Landing Page)', () => {
    test('Sarah discovers ESTA Tracker and feels hopeful', async ({ page }) => {
      ...
      const heroSection = page
        .locator('[data-testid="hero-section"], .hero, main')
        .first();

      // Look for TurboTax-style messaging (reassuring, simple, professional) - expanded patterns
      const pageContent = await page.textContent('body');
      const hasReassuring =
        /simple|easy|guide|help|confidence|trust|together|step-by-step|start|begin|manage|track/i.test(
          pageContent || ''
        );
      ...
      if (!hasReassuring) {
        console.log(
          'No reassuring language found. Body content sample:',
    ...
    test('Landing page builds trust before transaction', async ({ page }) => {
      ...
      const hasTrustSignals =
        /secure|privacy|protected|accurate|compliant|legal|certified|safe|trust|verified/i.test(
          bodyText || ''
        );
      ...
      if (!hasTrustSignals) {
        console.log(
          'No trust signals found. Body content sample:',
  ...
  test.describe('Act 3: Guided Journey (Setup Experience)', () => {
    test('Sarah experiences step-by-step guidance without overwhelm', async ({
      page,
    }) => {
      ...
      const buttonVisible = await managerButton
      ...
      const hasProgressIndicator = await page
        .locator('[data-testid*="step"], .step, [class*="progress"]')
        .first()
    ...
    test('Sarah gets reassurance and guidance throughout setup', async ({
      page,
    }) => {
  ...
  test.describe('Act 4: Confidence & Relief (First Success)', () => {
    test('Sarah sees completion confirmation and feels relief', async ({
      ...
      await page
        .locator('input[id="email"]')
        .fill(`sarah.quick.${Date.now()}@example.com`);
      ...
      await Promise.race([
        page
          .locator('text=/success|complete|done|ready|compliant/i')
  ...
  test.describe('Act 5: Ongoing Trust (Daily Use)', () => {
    test('Sarah can quickly check balances without complexity', async ({
      page,
    }) => {
      ...
      const hasBalanceInfo = await page
        .locator('[data-testid*="balance"], [class*="balance"]')
        .first()
        .isVisible({ timeout: 5000 })
        .catch(() => false);
  ...
  test.describe('Emotional Transformation Validation', () => {
    test('Complete journey demonstrates trust-building UX', async ({
      page,
    }) => {
      ...
      const usesPlainLanguage =
        !/statutory|regulatory framework|aggregate|compliance status: affirmative/i.test(
          bodyText || ''
        );
      ...
      if (!usesPlainLanguage) {
        console.log(
          'Technical jargon detected. Body content sample:',
          bodyText?.substring(0, 200)
        );
      }
...
test.describe("Sarah's Journey: UX Contract Validation", () => {
  ...
  test('Users always feel safe', async ({ page }) => {
    ...
    const hasSafetySignals =
      /secure|encrypt|privacy|protect|safe|trust|verified|compliant/i.test(
        bodyText || ''
      );
    ...
    if (!hasSafetySignals) {
      console.log(
        'No safety signals found. Body content sample:',
        bodyText?.substring(0, 200)
      );
    }
